<div align="center">

<img src="https://i.postimg.cc/bJ1N511k/file-00000000652c82108b4c170e410ab2f2.png" alt="VNDXS HUB" width="650"/>

<br/>

<img src="https://img.shields.io/badge/Version-1.0.0-blueviolet?style=for-the-badge&logo=github&logoColor=white"/>
<img src="https://img.shields.io/badge/Games-3_Supported-9b59b6?style=for-the-badge&logo=roblox&logoColor=white"/>
<img src="https://img.shields.io/badge/License-MIT-blue?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Open_Source-100%25-success?style=for-the-badge&logo=opensourceinitiative&logoColor=white"/>

<br/><br/>

[![Discord](https://img.shields.io/badge/Join_Discord-5865F2?style=for-the-badge&logo=discord&logoColor=white)](https://dsc.gg/vndxshub)

</div>

---

<div align="center">

## <img src="https://api.iconify.design/lucide:zap.svg?color=%231E90FF&width=28&height=28" valign="middle"/> Quick Start

</div>

```lua
loadstring(game:HttpGet("https://raw.githubusercontent.com/sd-community1/VNDXS-HUB/refs/heads/main/Loader.luau"))()
```

> The hub **auto-detects your game** — no configuration needed. Just execute and play.

---

<div align="center">

## <img src="https://api.iconify.design/lucide:shield-check.svg?color=%231E90FF&width=28&height=28" valign="middle"/> Open Source Philosophy

</div>

VNDXS HUB is **fully open source**. Every line of code is visible, auditable, and available for learning.

**You are free to:**
- Study the code and learn from it
- Fork and modify it for personal use
- Build your own hub on top of it

**You are NOT allowed to:**
- Claim you built this from scratch
- Remove or hide credit to **VNDXS HUB** and its developer **VNDXS**
- If you fork this project, you **must state clearly** that your project was derived from VNDXS HUB

> *"I received this project and took inspiration from VNDXS HUB by VNDXS — not the other way around."*  
> This is the only acceptable way to credit a fork.

---

<div align="center">

## <img src="https://api.iconify.design/lucide:gamepad-2.svg?color=%231E90FF&width=28&height=28" valign="middle"/> Supported Games

</div>

<table>
<tr>

<td align="center" width="33%">
<img src="https://i.postimg.cc/XJPm9PrB/Doors.webp" width="180" style="border-radius:12px"/><br/><br/>

**DOORS**

<img src="https://api.iconify.design/lucide:check-circle.svg?color=%2322c55e&width=14"/> Auto Farm &nbsp;
<img src="https://api.iconify.design/lucide:check-circle.svg?color=%2322c55e&width=14"/> Entity ESP<br/>
<img src="https://api.iconify.design/lucide:check-circle.svg?color=%2322c55e&width=14"/> Death Farm &nbsp;
<img src="https://api.iconify.design/lucide:check-circle.svg?color=%2322c55e&width=14"/> Item Notifier<br/>
<img src="https://api.iconify.design/lucide:check-circle.svg?color=%2322c55e&width=14"/> Remove Surge &nbsp;
<img src="https://api.iconify.design/lucide:check-circle.svg?color=%2322c55e&width=14"/> Lobby Support

</td>

<td align="center" width="33%">
<img src="https://i.postimg.cc/RZRMwM4c/no-Filter.webp" width="180" style="border-radius:12px"/><br/><br/>

**BLOX FRUITS**

<img src="https://api.iconify.design/lucide:check-circle.svg?color=%2322c55e&width=14"/> Auto Level (3 Seas)<br/>
<img src="https://api.iconify.design/lucide:check-circle.svg?color=%2322c55e&width=14"/> Full ESP · Kill Aura<br/>
<img src="https://api.iconify.design/lucide:check-circle.svg?color=%2322c55e&width=14"/> Teleport · Server Hop<br/>
<img src="https://api.iconify.design/lucide:check-circle.svg?color=%2322c55e&width=14"/> Auto Raid · Codes

</td>

<td align="center" width="33%">
<img src="https://i.postimg.cc/xTNYpvYT/Polish-20260910-074803292.jpg" width="180" style="border-radius:12px"/><br/><br/>

**ONE SHOT**

<img src="https://api.iconify.design/lucide:check-circle.svg?color=%2322c55e&width=14"/> Aimbot · Silent Aim<br/>
<img src="https://api.iconify.design/lucide:check-circle.svg?color=%2322c55e&width=14"/> Full ESP · Triggerbot<br/>
<img src="https://api.iconify.design/lucide:check-circle.svg?color=%2322c55e&width=14"/> No Recoil · Prediction<br/>
<img src="https://api.iconify.design/lucide:check-circle.svg?color=%2322c55e&width=14"/> Off-Screen Arrows

</td>

</tr>
</table>

---

<div align="center">

## <img src="https://api.iconify.design/lucide:crosshair.svg?color=%231E90FF&width=28&height=28" valign="middle"/> One Shot — Full Feature List

<img src="https://i.postimg.cc/xTNYpvYT/Polish-20260910-074803292.jpg" width="120" style="border-radius:10px"/>

</div>

<details>
<summary><img src="https://api.iconify.design/lucide:eye.svg?color=%231E90FF&width=16"/> &nbsp;<b>ESP System</b></summary>

<br/>

| Feature | Description |
|---|---|
| <img src="https://api.iconify.design/lucide:square-dashed.svg?color=%231E90FF&width=14"/> **Boxes** | Corner-style boxes, color-coded by visibility state |
| <img src="https://api.iconify.design/lucide:move-diagonal.svg?color=%231E90FF&width=14"/> **Tracers** | Lines from screen bottom to enemy position |
| <img src="https://api.iconify.design/lucide:person-standing.svg?color=%231E90FF&width=14"/> **Skeleton** | Full bone structure rendered on all players |
| <img src="https://api.iconify.design/lucide:info.svg?color=%231E90FF&width=14"/> **Info Overlay** | Name · Health Bar · Distance · Current Weapon |
| <img src="https://api.iconify.design/lucide:alert-triangle.svg?color=%23f59e0b&width=14"/> **Tactical Threat ESP** | Auto-colors: Red = High / Orange = Medium / Blue = Normal |
| <img src="https://api.iconify.design/lucide:arrow-right.svg?color=%231E90FF&width=14"/> **Off-Screen Arrows** | Edge indicators for enemies outside FOV, color = proximity |
| <img src="https://api.iconify.design/lucide:bomb.svg?color=%23ef4444&width=14"/> **Projectile ESP** | Tracks grenades and throwables with danger radius |

</details>

<details>
<summary><img src="https://api.iconify.design/lucide:crosshair.svg?color=%231E90FF&width=16"/> &nbsp;<b>Aimbot</b></summary>

<br/>

| Feature | Description |
|---|---|
| <img src="https://api.iconify.design/lucide:target.svg?color=%231E90FF&width=14"/> **Aimbot** | Camera locks to target via `hookmetamethod` — fully functional |
| <img src="https://api.iconify.design/lucide:circle.svg?color=%231E90FF&width=14"/> **FOV Circle** | Visual adjustable field-of-view ring |
| <img src="https://api.iconify.design/lucide:sliders.svg?color=%231E90FF&width=14"/> **Smoothness** | Controls snap speed to avoid sudden camera jumps |
| <img src="https://api.iconify.design/lucide:user.svg?color=%231E90FF&width=14"/> **Target Part** | Head / UpperTorso / HumanoidRootPart |
| <img src="https://api.iconify.design/lucide:wall.svg?color=%231E90FF&width=14"/> **Visibility Check** | Skips enemies behind walls |
| <img src="https://api.iconify.design/lucide:maximize-2.svg?color=%231E90FF&width=14"/> **Dynamic FOV** | Wider at close range · Narrower at long range |

</details>

<details>
<summary><img src="https://api.iconify.design/lucide:zap.svg?color=%231E90FF&width=16"/> &nbsp;<b>Silent Aim & Weapon Mods</b></summary>

<br/>

| Feature | Description |
|---|---|
| <img src="https://api.iconify.design/lucide:mouse-pointer-click.svg?color=%231E90FF&width=14"/> **Silent Aim** | Redirects bullets to target without moving camera |
| <img src="https://api.iconify.design/lucide:activity.svg?color=%231E90FF&width=14"/> **Triggerbot** | Auto-fires when crosshair is on an enemy |
| <img src="https://api.iconify.design/lucide:arrow-down.svg?color=%231E90FF&width=14"/> **No Recoil** | Real-time vertical recoil compensation |
| <img src="https://api.iconify.design/lucide:dot.svg?color=%231E90FF&width=14"/> **No Spread** | Eliminates bullet spread entirely |

</details>

<details>
<summary><img src="https://api.iconify.design/lucide:activity.svg?color=%231E90FF&width=16"/> &nbsp;<b>Predictive Aiming</b></summary>

<br/>

| Feature | Description |
|---|---|
| <img src="https://api.iconify.design/lucide:git-branch.svg?color=%231E90FF&width=14"/> **Prediction** | Calculates enemy velocity + bullet travel time |
| <img src="https://api.iconify.design/lucide:gauge.svg?color=%231E90FF&width=14"/> **Bullet Speed** | Adjustable 50–2000 studs/s to match any weapon |

</details>

---

<div align="center">

## <img src="https://api.iconify.design/lucide:swords.svg?color=%231E90FF&width=28&height=28" valign="middle"/> Blox Fruits — Full Feature List

<img src="https://i.postimg.cc/RZRMwM4c/no-Filter.webp" width="120" style="border-radius:10px"/>

</div>

<details>
<summary><img src="https://api.iconify.design/lucide:sword.svg?color=%231E90FF&width=16"/> &nbsp;<b>Auto Farm</b></summary>

<br/>

| Feature | Description |
|---|---|
| <img src="https://api.iconify.design/lucide:trending-up.svg?color=%231E90FF&width=14"/> **Auto Level** | Farms correct mob for your level automatically |
| <img src="https://api.iconify.design/lucide:map.svg?color=%231E90FF&width=14"/> **3 Sea Support** | First · Second · Third Sea — fully mapped |
| <img src="https://api.iconify.design/lucide:wind.svg?color=%231E90FF&width=14"/> **Kill Aura** | Auto-attacks nearby enemies |
| <img src="https://api.iconify.design/lucide:scroll.svg?color=%231E90FF&width=14"/> **Auto Quest** | Accepts and completes quests automatically |
| <img src="https://api.iconify.design/lucide:sword.svg?color=%231E90FF&width=14"/> **Weapon Select** | Melee / Sword / Blox Fruit |

</details>

<details>
<summary><img src="https://api.iconify.design/lucide:wrench.svg?color=%231E90FF&width=16"/> &nbsp;<b>Utility</b></summary>

<br/>

| Feature | Description |
|---|---|
| <img src="https://api.iconify.design/lucide:map-pin.svg?color=%231E90FF&width=14"/> **Teleport** | One-click teleport to any island |
| <img src="https://api.iconify.design/lucide:wifi.svg?color=%231E90FF&width=14"/> **Server Hop** | Auto-hops to a new server |
| <img src="https://api.iconify.design/lucide:shield.svg?color=%231E90FF&width=14"/> **Anti Ban** | Removes detection scripts |
| <img src="https://api.iconify.design/lucide:timer.svg?color=%231E90FF&width=14"/> **Anti AFK** | Prevents idle kick |
| <img src="https://api.iconify.design/lucide:gift.svg?color=%231E90FF&width=14"/> **Redeem Codes** | Redeems all active codes instantly |
| <img src="https://api.iconify.design/lucide:bar-chart.svg?color=%231E90FF&width=14"/> **Max Stats** | Allocates all stat points instantly |

</details>

---

<div align="center">

## <img src="https://api.iconify.design/lucide:door-open.svg?color=%231E90FF&width=28&height=28" valign="middle"/> DOORS — Full Feature List

<img src="https://i.postimg.cc/XJPm9PrB/Doors.webp" width="120" style="border-radius:10px"/>

</div>

<details>
<summary><img src="https://api.iconify.design/lucide:eye.svg?color=%231E90FF&width=16"/> &nbsp;<b>All Features</b></summary>

<br/>

| Feature | Description |
|---|---|
| <img src="https://api.iconify.design/lucide:bot.svg?color=%231E90FF&width=14"/> **Auto Farm** | Automates full hotel runs |
| <img src="https://api.iconify.design/lucide:scan-eye.svg?color=%231E90FF&width=14"/> **Entity ESP** | Highlights all entities through walls |
| <img src="https://api.iconify.design/lucide:package.svg?color=%231E90FF&width=14"/> **Item Notifier** | Alerts on important item spawns |
| <img src="https://api.iconify.design/lucide:skull.svg?color=%231E90FF&width=14"/> **Death Farm** | Automated death loop for XP/Knobs |
| <img src="https://api.iconify.design/lucide:zap-off.svg?color=%231E90FF&width=14"/> **Remove Surge** | Disables surge events |
| <img src="https://api.iconify.design/lucide:door-open.svg?color=%231E90FF&width=14"/> **Lobby Support** | Works in the DOORS lobby too |

</details>

---

<div align="center">

## <img src="https://api.iconify.design/lucide:palette.svg?color=%231E90FF&width=28&height=28" valign="middle"/> Theme

</div>

| Element | Hex | Color |
|---|---|---|
| **Text** | `#FFD700` | ![](https://img.shields.io/badge/-%23FFD700-FFD700?style=flat-square) |
| **Accent** | `#1E90FF` | ![](https://img.shields.io/badge/-%231E90FF-1E90FF?style=flat-square) |
| **Background** | `#050A1A` | ![](https://img.shields.io/badge/-%23050A1A-050A1A?style=flat-square) |
| **Main** | `#0A0F2E` | ![](https://img.shields.io/badge/-%230A0F2E-0A0F2E?style=flat-square) |
| **Outline** | `#1E3A8A` | ![](https://img.shields.io/badge/-%231E3A8A-1E3A8A?style=flat-square) |

> Theme is **locked** across all games — the VNDXS identity is non-negotiable.

---

<div align="center">

## <img src="https://api.iconify.design/lucide:folder-open.svg?color=%231E90FF&width=28&height=28" valign="middle"/> Project Structure

</div>

```
VNDXS-HUB/
│
├── Loader.luau                   ← Entry point — auto-detects game
│
├── Components/
│   ├── Environment.luau          ← Executor capability testing
│   ├── Interface.luau            ← UI library + forced theme
│   ├── InfoTab.luau              ← Info tab
│   ├── SettingsTab.luau          ← Settings + Privacy
│   ├── CreditsTab.luau           ← Credits tab with logo
│   ├── Analytics.luau            ← Opt-in only analytics
│   └── AdonisBypass.luau         ← Adonis admin bypass
│
├── Games/
│   ├── Doors/                    ← DOORS support
│   ├── BloxFruits/               ← Blox Fruits (3 seas)
│   ├── OneShot/                  ← One Shot combat features
│   └── Universal/                ← General mode fallback
│
└── Scripts/
    └── DeathFarm.luau
```

---

<div align="center">

## <img src="https://api.iconify.design/lucide:shield.svg?color=%231E90FF&width=28&height=28" valign="middle"/> Privacy & Safety

</div>

<img src="https://api.iconify.design/lucide:check.svg?color=%2322c55e&width=14"/> Analytics are **opt-in** — disabled by default  
<img src="https://api.iconify.design/lucide:check.svg?color=%2322c55e&width=14"/> **No IP collection** — ever  
<img src="https://api.iconify.design/lucide:check.svg?color=%2322c55e&width=14"/> **No HWID tracking** — ever  
<img src="https://api.iconify.design/lucide:check.svg?color=%2322c55e&width=14"/> Only collects: username · executor name · game name · execution count  
<img src="https://api.iconify.design/lucide:check.svg?color=%2322c55e&width=14"/> 100% open source — every line is auditable  

---

<div align="center">

## <img src="https://api.iconify.design/lucide:users.svg?color=%231E90FF&width=28&height=28" valign="middle"/> Credits

| <img src="https://api.iconify.design/lucide:crown.svg?color=%23FFD700&width=16"/> Role | Name |
|---|---|
| **Developer** | VNDXS |
| **UI Library** | Obsidian — deividcomsono |
| **Base Reference** | Abysall Hub — FireBacon (bocaj111004) |
| **BF Reference** | W_Skinny |

<br/>

<img src="https://i.postimg.cc/bJ1N511k/file-00000000652c82108b4c170e410ab2f2.png" width="300"/>

<br/>

[![Discord](https://img.shields.io/badge/Join_the_Community-5865F2?style=for-the-badge&logo=discord&logoColor=white)](https://dsc.gg/vndxshub)

<br/>

*VNDXS HUB — Built different.*

</div>
